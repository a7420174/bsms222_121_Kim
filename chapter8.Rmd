...
---
```{r setup, include=F}
knitr::opts_chunk$set(error = TRUE, echo = TRUE, fig.width = 5, fig.height = 3, fig.align = 'center')
options(digits = 3)
```
<br/>
...
```{r echo=FALSE}
data(murders)
murders %>% group_by(region) %>% count() %>% ungroup %>% mutate(Proportion = n/sum(n)) %>% ggplot(aes(region, Proportion, fill = region)) + geom_bar(stat = 'identity', show.legend = F) + xlab(NULL) + theme_bw()
```
<br/>
...
```{r echo=FALSE}
heights %>% filter(sex == 'Male') %>% ggplot(aes(height)) + stat_ecdf() + xlab("a") + ylab("F(a)") + theme_bw()
```
<br/>
...
```{r echo=FALSE}
heights %>% filter(sex == 'Male') %>% ggplot(aes(height)) + geom_histogram(binwidth = 1, closed = "right", col = "black") + theme_bw()
```
<br/>
...
```{r echo=FALSE}
heights %>% filter(sex == "Male") %>% ggplot(aes(height)) + geom_density(fill = 'cadetblue2', alpha = 0.3) + theme_bw()
```
<br/>
...
```{r echo=FALSE}
x <- rnorm(1000000, 67, sd(heights$height))
data.frame(height = x) %>% ggplot(aes(height)) + geom_histogram(binwidth = 1, col = "black") + theme_bw()
```
<br/>
...
library(gridExtra)
a <- data.frame(height = x) %>% ggplot(aes(height)) + geom_histogram(binwidth = 1, col = "black") + ggtitle("binwidth=1") + theme_bw()
b <- data.frame(height = x) %>% ggplot(aes(height)) + geom_histogram(binwidth = 0.5, col = "black") + ggtitle("binwidth=0.5") + theme_bw()
c <- data.frame(height = x) %>% ggplot(aes(height)) + geom_histogram(binwidth = 0.1, col = "black") + ggtitle("binwidth=0.1") + theme_bw()
...
```{r echo=F}
data.frame(height = x) %>% ggplot(aes(height, ..density..)) + geom_histogram(binwidth = 0.1, col = "black") + theme_bw()
```
<br/>
...
a <- heights %>% filter(sex == 'Male') %>% ggplot(aes(height, ..density..)) + geom_histogram(binwidth = 1, closed = "right", col = "black") + theme_bw()
midpoint <- function(range){
  str_extract_all(range, '\\d+[.]\\d') %>% unlist %>% as.numeric %>% mean
}
df <- heights %>% filter(sex == 'Male') %>% mutate(range = cut_width(height, width = 1)) %>% group_by(range) %>% count() %>% ungroup() %>% mutate(prop = n/sum(n), midpoint = sapply(range, midpoint))
df2 <- data.frame(midpoint = 50:83) %>% mutate(prop = ifelse(midpoint %in% df$midpoint, NA, 0))
b <- a + geom_point(data = df, aes(midpoint, prop), col = "blue", size = 2) + geom_point(data = df2, aes(midpoint, prop), col = "blue", size = 2)
c <- ggplot() + geom_point(data = df, aes(midpoint, prop), col = "blue", size = 2) + geom_point(data = df2, aes(midpoint, prop), col = "blue", size = 2) + xlab("height") + ylab("density") + theme_bw()
d <- heights %>% filter(sex == "Male") %>% ggplot(aes(height)) + geom_density() + theme_bw() + geom_point(data = df, aes(midpoint, prop), col = "blue", size = 2) + geom_point(data = df2, aes(midpoint, prop), col = "blue", size = 2) + theme_bw()
e <- a + geom_density()
f <- heights %>% filter(sex == "Male") %>% ggplot(aes(height)) + geom_density(fill = 'cadetblue2', alpha = 0.3) + theme_bw()
...
```{r echo=F}
a <- heights %>% filter(sex == 'Male') %>% ggplot(aes(height, ..density..)) + geom_histogram(binwidth = 1, closed = "right", alpha = 0.5) + geom_density(adjust = 0.5) + theme_bw()
b <- heights %>% filter(sex == 'Male') %>% ggplot(aes(height, ..density..)) + geom_histogram(binwidth = 1, closed = "right", alpha = 0.5) + geom_density() + theme_bw()
...
```{r echo=F}
dens <- heights %>% filter(sex == "Male") %>% .$height %>% density()
area <- data.frame(height = dens[["x"]][between(dens[["x"]],65,68)], density = dens[["y"]][between(dens[["x"]],65,68)])
heights %>% filter(sex == "Male") %>% ggplot(aes(height)) + geom_density() + theme_bw() + geom_area(data = area, aes(x = height, y = density), fill = 'cadetblue2', alpha = 0.3)
```
<br/>
...
```{r echo=F}
heights %>% ggplot(aes(height, fill = sex)) + geom_density(alpha = 0.2) + theme_bw()
```
<br/>
...
```{r echo=F}
murders %>% ggplot(aes(reorder(region, region, length), ..count../sum(..count..), fill = region)) + geom_bar(show.legend = F) + xlab('region') + ylab('prop') + theme_bw()
```
<br/>

To the closest 5%, what proportion of the states are in the North Central region?
```{r}
murders %>% count(region) %>% mutate(prop = n/sum(n)) %>% filter(region == "North Central")
```
<br/>
...
```{r echo=F}
murders %>% mutate(rate = total/population*100000) %>% ggplot(aes(rate)) + stat_ecdf() + xlab("a") + ylab("F(a)") + theme_bw()
```
<br/>
...
```{r}
murders %>% mutate(rate = total/population*100000) %>% summarise(ans = mean(rate > 10))
```
...
murders %>% mutate(rate = total/population*100000) %>% 
  filter(rate >= 5) %>% count(name = "exception")
```
...
Based on this plot, how many males are between 64.5 and 65.5?

```{r}
heights %>% filter(sex == 'Male') %>% summarise(ans = sum(between(height, 63.5, 65.5)))
```
...
```{r}
heights %>% filter(sex == 'Male') %>% summarise(ans = mean(height < 60))
```
...
```{r echo=F}
murders %>% mutate(pop_in_millions = population/1000000) %>% ggplot(aes(pop_in_millions)) + geom_density(fill = "gray") + theme_bw() + scale_x_continuous(trans = "log2", breaks = c(1, 3, 10, 30)) + xlab("Population in millions")
```
<br/>

```{r}
murders %>% mutate(pop_in_millions = population/1000000) %>% 
  summarize(ans = mean(pop_in_millions > 10))
```
...
```{r echo=F}
p1 <- murders %>% mutate(pop_in_millions = population/1000000) %>% ggplot(aes(pop_in_millions)) + geom_density(fill = "gray", adjust = 3) + theme_bw() + scale_x_continuous(breaks = c(10, 20, 30)) + xlab("Population in millions") + ggtitle("1")
p2 <- murders %>% mutate(pop_in_millions = population/1000000) %>% ggplot(aes(pop_in_millions)) + geom_density(fill = "gray", adjust = 0.27) + theme_bw() + scale_x_continuous(trans = "log2", breaks = c(1, 3, 10, 30)) + xlab("Population in millions") + ggtitle("2")
p3 <- murders %>% mutate(pop_in_millions = population/1000000) %>% ggplot(aes(pop_in_millions)) + geom_density(fill = "gray", adjust = 5) + theme_bw() + scale_x_continuous(trans = "log2", breaks = c(1, 3, 10, 30)) + xlab("Population in millions") + ggtitle("3")
...
$$Pr(a<x<b)=\int_a^b\frac1{\sqrt{2\pi}s}e^{-\frac12(\frac{x-m}s)^2}dx$$
...
```{r echo=F}
ggplot(tibble(x=c(-4,4)), aes(x)) + stat_function(fun = dnorm) + ylab('density') + theme_bw()
```
<br/>
...
```{r echo=F}
heights %>% filter(sex == 'Male') %>% ggplot(aes(height)) + geom_density(fill = 'dodgerblue') + stat_function(data = tibble(x = c(55, 83.5)), aes(x), fun = dnorm, args = c(69.3, 3.6), size = 1.2) + theme_bw()
```
<br/>
...
```{r echo=F}
a <- murders %>% mutate(rate = total/population*100000) %>% ggplot(aes(rate)) + geom_histogram(binwidth = 1) + ggtitle("Histogram") + theme_bw()
params <- murders %>% mutate(rate = total/population*100000) %>% summarize(avg = mean(rate), sd = sd(rate))
b <- murders %>% mutate(rate = total/population*100000) %>% ggplot(aes(sample = rate)) + geom_qq(dparams = c(params$avg, params$sd)) + geom_abline() + ggtitle("QQ-plot") + theme_bw()
grid.arrange(a,b, nrow = 1)
```
<br/>
...
```{r echo=F}
murders %>% mutate(rate = total/population*100000) %>% ggplot(aes(y= rate)) + geom_boxplot() + theme_bw() + scale_x_continuous(breaks = 0, label = NULL,expand = c(2,0))
```
<br/>
...
```{r echo=F}
heights %>% ggplot(aes(sex, height, fill = sex)) + geom_boxplot() + theme_bw()
```
<br/>
...
```{r echo=F}
a <- heights %>% filter(sex == 'Female') %>% ggplot(aes(height, fill = sex)) + geom_density(show.legend = F) + theme_bw()
b <- heights %>% filter(sex == 'Female') %>% ggplot(aes(sample = scale(height))) + geom_qq() + ylab("Standard Units") + geom_abline() + theme_bw()
grid.arrange(a,b, nrow = 1)
```
<br/>
...
heights %>% group_by(sex) %>% 
  summarize(percentile = quantile(height, seq(0.1, 0.9, 0.2))) %>% 
  ungroup %>% mutate(percent = names(percentile)) %>% spread(sex, percentile)
```
<br/>
3. Study the following boxplots showing population sizes by country:

```{r echo=F}
gapminder %>% filter(!is.na(population)&year == "2010") %>% mutate(pop_in_millions = population/10^6) %>% ggplot(aes(continent, pop_in_millions)) + geom_boxplot() + ylab("Population in millions") + scale_y_continuous(breaks = c(1, 10, 100, 1000), trans = "log10") + theme_bw()
```
<br/>
...
```{r}
actual <- mean(x>79 & x<=81)
approximation <- pnorm(81, mean(x), sd(x)) - pnorm(79, mean(x), sd(x))
...
```{r echo=F}
heights %>% ggplot(aes(sex, height)) + geom_boxplot() + theme_bw()
```
<br/>
...
## **8.17 Exercises**
<br/>
1. Now we are going to use the `geom_histogram` function to make a histogram of the heights in the `heights` data frame. When reading the documentation for this function we see that it requires just one mapping, the values to be used for the histogram. Make a histogram of all the plots.
