...
```{r echo=F}
gapdf <- gapminder %>% filter(country %in% c("Germany", "South Korea") & year %in% c(1960, 1961, 1962)) %>% select(country, year, fertility)
gapdf
```
<br/>
...
```{r echo=F}
gapdf %>% spread(key = year, value = fertility)
```
<br/>
...
<br/>3. Examine the built-in dataset `BOD`. Which of the following is true:
```{r}
BOD
```
...
<br/>4. Which of the following built-in datasets is tidy (you can pick more than one):
...
```{r}
ref <- NHANES %>% filter(AgeDecade == ' 20-29') %>% summarise(mean(BPSysAve, na.rm = TRUE), sd(BPSysAve, na.rm = TRUE))
ref
```
<br/>
2. Using a pipe, assign the average to a numeric variable `ref_avg`. Hint: Use the code similar to above and then `pull`.
```{r}
ref_avg <- NHANES %>% filter(AgeDecade == ' 20-29') %>% summarise(avg = mean(BPSysAve, na.rm = TRUE), sd = sd(BPSysAve, na.rm = TRUE)) %>% pull(avg)
ref_avg
```
<br/>
3. Now report the min and max values for the same group.
```{r}
NHANES %>% filter(AgeDecade == ' 20-29') %>% summarise(min = min(BPSysAve, na.rm = TRUE), max = max(BPSysAve, na.rm = TRUE))
```
<br/>
...
```{r}
NHANES %>% filter(Gender == 'female') %>% group_by(AgeDecade) %>% summarise(avg = mean(BPSysAve, na.rm = TRUE), sd = sd(BPSysAve, na.rm = TRUE))
```
<br/>
5. Repeat exercise 4 for males.
```{r}
NHANES %>% filter(Gender == 'male') %>% group_by(AgeDecade) %>% summarise(avg = mean(BPSysAve, na.rm = TRUE), sd = sd(BPSysAve, na.rm = TRUE))
```
<br/>
...
```{r}
NHANES %>% group_by(AgeDecade, Gender) %>% summarise(avg = mean(BPSysAve, na.rm = TRUE), sd = sd(BPSysAve, na.rm = TRUE))
```
<br/>
...
```{r}
Race1 <- NHANES %>% filter(AgeDecade == " 40-49" & Gender == "male") %>% group_by(Race1) %>% summarise(avg = mean(BPSysAve, na.rm = TRUE), sd = sd(BPSysAve, na.rm = TRUE)) %>% arrange(desc(avg))
Race1
```
<br/>
...
```{r}
murders_group <- murders %>% group_by(region)
class(murders_group)
```
<br/>
...
```{r}
compute_s_n <- function(n){
  tibble(n = n, s_n = n*(n+1)/2, s_n_2 = sum(1:n))
}
