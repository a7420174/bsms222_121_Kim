...
```{r}
dat <- read_csv(fullpath[5], col_names = F, skip = 1)
```
<br/>
...
```{r}
colns <- read_lines(fullpath[5], n_max = 1)
colns <- colns %>%  strsplit(",") %>% unlist %>% .[2:length(.)]
dat <- read_csv(fullpath[5], col_names = F, skip = 1) %>% select(-X1)
colnames(dat) <- c("Region_char", colns)
head(dat)
```
<br/>
...
```{r}
fullpath <- file.path("C:/Users/Kim JaeHyun/Documents/bsms222_121_kim", "mandarine.csv")
mand <- read_csv(fullpath, col_names = c("Date", "Yield"), skip = 1)
plot(mand$Date,mand$Yield, type = "h", xlab = "Date", ylab = "Yield")
```
