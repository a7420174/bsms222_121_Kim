...
---
```{r setup, include=F}
knitr::opts_chunk$set(error = TRUE, echo = TRUE, fig.width = 6, fig.height = 4, fig.align = 'center')
options(digits = 3)
```
<br>
...
```{r echo=F}
country1 <- c("Sri Lanka","Poland","Malaysia","Pakistan","Thailand")
country2 <- c('Turkey','South Korea','Russia','Vietnam','South Africa')
mor1 <- c(8.4,4.5,6.0,65.8,10.5)
mor2 <- c(11.6,2.9,8.2,17.3,33.6)
tb <- tibble(country1 = country1, mor1 = mor1, country2 = country2, mor2 = mor2)
colnames(tb) <- c('country','infant mortality','country','infant mortality')
knitr::kable(tb)
```
<br>
...
### **9.5.2 Which base?**
In the case above, we used base 2 in the log transformations. Other common choices are base $e$ (the natural log) and base 10In general, we do not recommend using the natural log for data exploration and visualization. This is because while $2^2,2^3,2^4,…$ or $10^2,10^3,…$ are easy to compute in our heads, the same is not true for $e^2,e^3,…$, so the scale is not intuitive or easy to interpret.
...
```{r echo=F}
x <- c(rnorm(250, sd = 1),rnorm(100, mean = 5))
p1 <- ggplot(NULL, aes(x)) + geom_density(fill = '#9AB2C8', alpha = 0.7) + theme_bw() + scale_x_continuous(breaks = c(0,5)) + ylab(NULL)
p2 <- ggplot(NULL, aes(x = "",y = x)) + geom_boxplot() + theme_bw() + xlab(NULL)
plot_grid(p1,p2)
```
<br>
...
gapminder %>% 
  ...
  mutate(per_incr = (`2010`-`1970`)/`1970`*100, group = reorder(group, per_incr, median)) %>%
  ggplot(aes(group, per_incr)) + geom_boxplot() + geom_point() + xlab("") + 
  ylab("Percent increase: 1970 to 2010") + theme(axis.text.x = element_text(angle = 1, hjust = 1)) + theme_bw()
```
<br>
...
```{r echo = F}
gapminder %>% 
  filter(year %in% past_year & !is.na(gdp) & country %in% country_list) %>%
  mutate(west = ifelse(group == "West", "West", "Developing")) %>% 
  count(west) %>% knitr::kable()
```
<br>
...
library(ggrepel)
gapminder %>% 
  filter(year == 2011 & !is.na(dollars_per_day) & !is.na(infant_mortality)) %>% mutate(survival = 1 - infant_mortality/1000, group = case_when(
...
.$region %in% c('Melanesia', 'Micronesia', 'Polynesia') ~ 'Pacific Islands')) %>% group_by(group) %>% summarise(infant_survival_rate = sum(survival*population/sum(population)), income = sum(dollars_per_day * population/sum(population))) %>% ggplot(aes(income, infant_survival_rate, label = group, color = group)) + geom_label_repel(show.legend = F) + scale_x_continuous(trans = 'log2', breaks = c(1, 8, 64), limits = c(1/5, 200), expand = c(0,0)) + scale_y_continuous(trans = 'logit', breaks = c(0.850, 0.900, 0.950, 0.990, 0.995, 0.998), limits = c(0.85, 0.9983), expand = c(0,0)) + theme_bw()
```
<br>
...
```{r echo=F}
gapminder %>% 
  filter(year == 2010 & !is.na(dollars_per_day) & !is.na(infant_mortality)) %>% mutate(infant_survival_rate = 1 - infant_mortality/1000, group = case_when(
.$region %in% c('Western Europe', 'Northern Europe', 'Southern Europe', 'Northern America', 'Australia and New Zealand') ~ 'West',
.$region %in% 'Northern Africa' ~ 'Northern Africa',
.$region %in% c('Eastern Asia', 'South-Eastern Asia') ~ 'East Asia',
.$region %in% 'Southern Asia' ~ 'Southern Asia',
